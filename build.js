...
const addonsDirectoryName = "~/Documents/Simutrans/addons/pak128.cz";
...
getDirectories(__dirname)
  .filter(name => !name.includes("_") && !name.includes(".git"))
  .forEach(dir => {
    const pakName = `pak128cz-${path.basename(dir)}.pak`;
    const pakPaths = fs
      .readdirSync(dir)
      .filter(item => item.endsWith(".dat"))
      .map(dat => {
        const datPakName = `pak128cz-intermidiate-${path.basename(
          dir
        )}-${dat}.pak`;
        execSync(
          `wine ${makeObjLocation} QUIET pak128 ${path.join(
            buildDirectoryName,
            datPakName
          )} ${path.join(dir, dat)}`
        );
    ...
    execSync(
      `wine ${makeObjLocation} QUIET MERGE ${path.join(
        buildDirectoryName,
        pakName
      )} ${pakPaths.join(" ")}`
    );
